...
namespace T1
{
    T1SteppingAction::T1SteppingAction(T1EventAction *T1EventAction)
        : G4UserSteppingAction(),
          fEventAction(fEventAction),
    ...
    void T1SteppingAction::UserSteppingAction(const G4Step *aStep)
    {
        ...
        {
            const auto detectorConstruction = static_cast<const T1DetectorConstruction>(G4RunManager::GetRunManager()->GetUserDetectorConstruction());
        }
        ...
        {
            if (currentVolume == fScoringVolumeVector[i])
                IsScoringVolume = true;
        }
