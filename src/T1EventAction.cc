...
namespace T1
{
    T1EventAction::T1EventAction(T1RunAction *runAction) : fRunAction(runAction),
                                                           fTotalEdep(0.)
    {
    }
    ...
    void T1EventAction::BeginOfEventAction(const G4Event *anEvent)
    {
        ...
        G4int nEvents = G4RunManager::GetRuManager()->GetCurrentRun()->GetNumberOfEventToBeProcessed();
        if (nEvents > 10)
        {
